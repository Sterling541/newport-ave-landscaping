/* ============================================================
   SALES REPS CONTENT (no AdminLayout wrapper)
   Used inside Configuration page.
   ============================================================ */
import { useState } from "react";
import { trpc } from "@/lib/trpc";
import {
  UserCheck, Plus, Edit2, Check, X, RefreshCw, Calendar, Mail, Phone, AlertCircle,
} from "lucide-react";
...
function ToastMsg({ message, onClose }: { message: string; onClose: () => void }) {
  return (
...
const COLOR_SWATCHES = [
  ...
  { value: "#f59e0b", label: "Amber" }, { value: "#a855f7", label: "Purple" },
  { value: "#ef4444", label: "Red" }, { value: "#14b8a6", label: "Teal" },
  { value: "#f97316", label: "Orange" }, { value: "#ec4899", label: "Pink" },
];
...
export default function SalesRepsContent() {
  ...
  const createMutation = trpc.scheduler.createRep.useMutation({
    onSuccess: () => { showToast("Sales rep created."); setShowAddForm(false); setForm(EMPTY_FORM); refetch(); },
    onError: (err) => showToast(`Error: ${err.message}`),
  });

  const updateMutation = trpc.scheduler.updateRep.useMutation({
    onSuccess: () => { showToast("Sales rep updated."); setEditingId(null); refetch(); },
    onError: (err) => showToast(`Error: ${err.message}`),
  });
  ...
  function submitCreate() {
    ...
    createMutation.mutate({ name: form.name.trim(), role: form.role,
  ...
  function toggleActive(rep: typeof reps[0]) {
    updateMutation.mutate({ id: rep.id, isActive: !rep.isActive });
  }
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-5">
        <p className="text-sm text-slate-500">Manage sales consultants for the Smart Scheduler</p>
        <div className="flex items-center gap-2">
          <button onClick={() => setShowInactive(!showInactive)}
            className="px-3 py-1.5 rounded-lg text-sm border transition-colors"
            style={{ borderColor: showInactive ? "oklch(0.55 0.15 145)" : "oklch(0.85 0.02 155)",
          ...
          <button onClick={() => { setShowAddForm(!showAddForm); setForm(EMPTY_FORM); }}
      ...
      {/* Add Form */}
      {showAddForm && (
        <div className="mb-6 p-5 rounded-2xl border" style={{ background: "oklch(0.97 0.01 145)", borderColor: "oklch(0.88 0.04 145)" }}>
          <h3 className="font-semibold mb-4" style={{ color: "oklch(0.25 0.08 155)" }}>Add New Sales Rep</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value as "install_design" | "enhancement" })}
            ...
            <div>
              ...
              <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })}
                placeholder="email@example.com" className="w-full px-3 py-2 rounded-lg border text-sm outline-none"
                style={{ borderColor: "oklch(0.82 0.03 155)" }} />
            </div>
            <div>
              <label className="block text-xs font-medium mb-1" style={{ color: "oklch(0.45 0.05 155)" }}>Phone</label>
              <input type="tel" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })}
                placeholder="(541) 555-0100" className="w-full px-3 py-2 rounded-lg border text-sm outline-none"
                style={{ borderColor: "oklch(0.82 0.03 155)" }} />
            </div>
            <div className="sm:col-span-2">
              <label className="block text-xs font-medium mb-1" style={{ color: "oklch(0.45 0.05 155)" }}>
                Google Calendar ID <span className="ml-1 font-normal opacity-60">(optional)</span>
              </label>
              <input type="text" value={form.googleCalendarId} onChange={(e) => setForm({ ...form, googleCalendarId: e.target.value })}
                placeholder="example@group.calendar.google.com" className="w-full px-3 py-2 rounded-lg border text-sm outline-none font-mono"
                style={{ borderColor: "oklch(0.82 0.03 155)" }} />
            </div>
            <div className="sm:col-span-2">
              <label className="block text-xs font-medium mb-2" style={{ color: "oklch(0.45 0.05 155)" }}>Calendar Color</label>
              <div className="flex flex-wrap gap-2">
                {COLOR_SWATCHES.map((swatch) => (
                  <button key={swatch.value} type="button" onClick={() => setForm({ ...form, calendarColor: swatch.value })}
                    title={swatch.label} className="w-7 h-7 rounded-full border-2 transition-transform hover:scale-110"
                ...
                {form.calendarColor && (
                  <button type="button" onClick={() => setForm({ ...form, calendarColor: "" })}
                    className="w-7 h-7 rounded-full border-2 text-xs flex items-center justify-center"
                    style={{ borderColor: "oklch(0.82 0.03 155)", color: "oklch(0.60 0.04 155)" }} title="Clear">✕</button>
                )}
              </div>
            </div>
          </div>
          <div className="flex gap-2 mt-4">
            <button onClick={submitCreate} disabled={createMutation.isPending || !form.name.trim()}
              className="px-4 py-2 rounded-lg text-sm font-medium text-white disabled:opacity-50"
              style={{ background: "oklch(0.45 0.15 145)" }}>
              {createMutation.isPending ? "Saving…" : "Save Rep"}
            </button>
            <button onClick={() => setShowAddForm(false)} className="px-4 py-2 rounded-lg text-sm border"
              style={{ borderColor: "oklch(0.82 0.03 155)", color: "oklch(0.50 0.05 155)" }}>Cancel</button>
          </div>
        </div>
      )}
      ...
      <div className="mb-5 flex items-start gap-3 p-4 rounded-xl border"
        ...
        <div className="text-sm" style={{ color: "oklch(0.40 0.10 75)" }}>
          <strong>Google Calendar integration is pending setup.</strong> Once you complete the Google Cloud setup, paste each rep's Calendar ID below.
        </div>
      </div>

      {/* Reps List */}
      {isLoading ? (
        <div className="text-center py-12 text-sm" style={{ color: "oklch(0.60 0.04 155)" }}>Loading reps…</div>
      ) : reps.length === 0 ? (
        <div className="text-center py-16 rounded-2xl border border-dashed" style={{ borderColor: "oklch(0.82 0.03 155)" }}>
          ...
          <button onClick={() => seedMutation.mutate()} className="px-4 py-2 rounded-lg text-sm font-medium text-white"
            style={{ background: "oklch(0.45 0.15 145)" }}>Seed Default Reps</button>
        </div>
      ) : (
        <div className="space-y-3">
          {reps.map((rep) => (
            <div key={rep.id} className="rounded-2xl border p-5 transition-shadow hover:shadow-sm"
              style={{ background: rep.isActive ? "white" : "oklch(0.97 0.005 155)",
              ...
              {editingId === rep.id ? (
                <div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                      <label className="block text-xs font-medium mb-1" style={{ color: "oklch(0.45 0.05 155)" }}>Role</label>
                      <select value={editForm.role} onChange={(e) => setEditForm({ ...editForm, role: e.target.value as "install_design" | "enhancement" })}
                        className="w-full px-3 py-2 rounded-lg border text-sm outline-none" style={{ borderColor: "oklch(0.82 0.03 155)" }}>
                        <option value="install_design">Install / Design</option>
                        <option value="enhancement">Enhancement</option>
                      </select>
                    ...
                    <div className="sm:col-span-2">
                      <label className="block text-xs font-medium mb-1" style={{ color: "oklch(0.45 0.05 155)" }}>Google Calendar ID</label>
                    ...
                    <div className="sm:col-span-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {COLOR_SWATCHES.map((swatch) => (
                          <button key={swatch.value} type="button" onClick={() => setEditForm({ ...editForm, calendarColor: swatch.value })}
                            title={swatch.label} className="w-7 h-7 rounded-full border-2 transition-transform hover:scale-110"
                            style={{ background: swatch.value,
                        ...
                        {editForm.calendarColor && (
                          <button type="button" onClick={() => setEditForm({ ...editForm, calendarColor: "" })}
                            className="w-7 h-7 rounded-full border-2 text-xs flex items-center justify-center"
                            style={{ borderColor: "oklch(0.82 0.03 155)", color: "oklch(0.60 0.04 155)" }} title="Clear">✕</button>
                        )}
                      </div>
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => submitEdit(rep.id)} disabled={updateMutation.isPending}
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium text-white"
                      style={{ background: "oklch(0.45 0.15 145)" }}>
                      <Check className="w-3.5 h-3.5" />
                      {updateMutation.isPending ? "Saving…" : "Save"}
                    </button>
                    <button onClick={() => setEditingId(null)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm border"
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-4">
                    <div className="w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold shrink-0"
                      style={{ background: ROLE_COLORS[rep.role]?.bg ?? "oklch(0.92 0.05 155)", color: ROLE_COLORS[rep.role]?.text ?? "oklch(0.30 0.10 155)" }}>
                      {rep.name.split(" ").map((n) => n[0]).join("").slice(0, 2)}
                    </div>
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-semibold text-sm" style={{ color: "oklch(0.20 0.05 155)" }}>{rep.name}</span>
                        ...
                        {!rep.isActive && (
                          <span className="px-2 py-0.5 rounded-full text-xs font-medium"
                            style={{ background: "oklch(0.93 0.03 0)", color: "oklch(0.45 0.10 0)" }}>Inactive</span>
                        )}
                      </div>
                      <div className="mt-1.5 space-y-0.5">
                        ...
                        {rep.googleCalendarId ? (
                          ...
                          <div className="flex items-center gap-1.5 text-xs" style={{ color: "oklch(0.60 0.08 75)" }}><Calendar className="w-3 h-3" /><span>No Calendar ID</span></div>
                        )}
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <button onClick={() => startEdit(rep)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs border transition-colors hover:bg-gray-50"
                      style={{ borderColor: "oklch(0.85 0.02 155)", color: "oklch(0.50 0.05 155)" }}>
                      <Edit2 className="w-3 h-3" /> Edit
                    </button>
                    <button onClick={() => toggleActive(rep)} disabled={updateMutation.isPending}
                      className="px-3 py-1.5 rounded-lg text-xs border transition-colors"
                      style={{ borderColor: rep.isActive ? "oklch(0.88 0.06 0)" : "oklch(0.82 0.03 155)",
      ...
      {toastMsg && <ToastMsg message={toastMsg} onClose={() => setToastMsg(null)} />}
    </div>
  );
}
